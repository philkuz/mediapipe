...
std::string CameraMotionFlagToString(const CameraMotion& camera_motion) {
  ...
  if (camera_motion.flags() & CameraMotion::CAMERA_MOTION_TYPE_FLAG_BLURRY_FRAME) {
    text += absl::StrFormat("BLURRY_FRAME %.2f|", camera_motion.bluriness());
  }

  if (camera_motion.flags() & CameraMotion::CAMERA_MOTION_TYPE_FLAG_MAJOR_OVERLAY) {
    text += "MAJOR_OVERLAY|";
  }

  if (camera_motion.flags() & CameraMotion::CAMERA_MOTION_TYPE_FLAG_SHARP_FRAME) {
    text += "SHARP_FRAME|";
  }

  if (camera_motion.flags() & CameraMotion::CAMERA_MOTION_TYPE_FLAG_SHOT_FADE) {
    text += "SHOT_FADE|";
  }

  if (camera_motion.flags() & CameraMotion::CAMERA_MOTION_TYPE_FLAG_DUPLICATED) {
    text += "DUPLICATED|";
  }
  return text;
}
